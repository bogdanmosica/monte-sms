...
const mockStaffData = {
  totalStaff: mockUsers.filter((user) => user.role === UserRole.TEACHER).length + 1, // +1 for admin
  teachers: mockUsers.filter((user) => user.role === UserRole.TEACHER).length,
  activeStaff: mockUsers.filter((user) => user.role === UserRole.TEACHER).length + 1,
};
