...
async function getChildWithAccess(
  childId: number,
  userId: number,
  userRole: string
) {
  let whereClause = and(eq(children.id, childId), eq(children.deletedAt, null));
      ...
      async (req, user) => {
        ...
        if (!child) {
          logAuthEvent(
            'ACCESS_DENIED',
            user.id,
            `Attempted to access child ${childId}`
          );
        ...
        logAuthEvent(
          'LOGIN_SUCCESS',
          user.id,
          `Retrieved child: ${child.firstName} ${child.lastName}`
        );
  ...
  try {
    return await withRole(
      ...
      async (req, user) => {
        ...
        const existingChild = await getChildWithAccess(
          childId,
          user.id,
          user.role
        );
        if (!existingChild) {
          logAuthEvent(
            'ACCESS_DENIED',
            user.id,
            `Attempted to update child ${childId}`
          );
          return Response.json(
            { error: 'Child not found or access denied' },
            { status: 404 }
        ...
        logAuthEvent(
          'LOGIN_SUCCESS',
          user.id,
          `Updated child: ${updatedChild.firstName} ${updatedChild.lastName}`
        );
  ...
  try {
    return await withRole([UserRole.ADMIN], async (req, user) => {
      ...
      const [deletedChild] = await db
        ...
        .where(and(eq(children.id, childId), eq(children.deletedAt, null)))
        .returning();
      ...
      logAuthEvent(
        'LOGIN_SUCCESS',
        user.id,
        `Deleted child: ${deletedChild.firstName} ${deletedChild.lastName}`
      );
