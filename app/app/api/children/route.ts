  ...
  try {
    return await withRole([UserRole.PARENT, UserRole.TEACHER, UserRole.ADMIN], async (req, user) => {
      ...
      if (user.role === UserRole.PARENT) {
        ...
        if (schoolId) {
          whereClause = and(whereClause, eq(children.schoolId, parseInt(schoolId)));
        }
        ...
        if (parentId) {
          whereClause = and(whereClause, eq(children.parentId, parseInt(parentId)));
        }
      }
      ...
      logAuthEvent('LOGIN_SUCCESS', user.id, `Retrieved ${childrenList.length} children`);
...
export async function POST(request: NextRequest) {
  try {
    return await withRole([UserRole.TEACHER, UserRole.ADMIN], async (req, user) => {
      ...
      if (!parent.length) {
        return Response.json({ error: 'Parent not found or invalid role' }, { status: 404 });
      }
      ...
      logAuthEvent('LOGIN_SUCCESS', user.id, `Created child: ${newChild.firstName} ${newChild.lastName}`);
