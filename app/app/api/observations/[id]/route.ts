...
async function getObservationWithAccess(
  ...
  try {
    return await withRole(
      ...
      async (req, user) => {
        ...
        const observation = await getObservationWithAccess(
          observationId,
          user.id,
          user.role
        );
        ...
        if (!observation) {
          logAuthEvent(
            'ACCESS_DENIED',
            user.id,
            `Attempted to access observation ${observationId}`
          );
          return Response.json(
        ...
        logAuthEvent(
          'LOGIN_SUCCESS',
          user.id,
          `Retrieved observation: ${observation.title}`
        );
      ...
      async (req, user) => {
        ...
        if (!existingObservation) {
          logAuthEvent(
            'ACCESS_DENIED',
            user.id,
            `Attempted to update observation ${observationId}`
          );
          return Response.json(
            { error: 'Observation not found or access denied' },
            { status: 404 }
        ...
        if (validatedData.skillsDemonstrated) {
          updateData.skillsDemonstrated = JSON.stringify(
            validatedData.skillsDemonstrated
          );
        }
        if (validatedData.developmentalMilestones) {
          updateData.developmentalMilestones = JSON.stringify(
            validatedData.developmentalMilestones
          );
        }
        if (validatedData.materialsUsed) {
          updateData.materialsUsed = JSON.stringify(
            validatedData.materialsUsed
          );
        }
        ...
        logAuthEvent(
          'LOGIN_SUCCESS',
          user.id,
          `Updated observation: ${updatedObservation.title}`
        );
      ...
      async (req, user) => {
        ...
        if (!deletedObservation) {
          logAuthEvent(
            'ACCESS_DENIED',
            user.id,
            `Attempted to delete observation ${observationId}`
          );
          return Response.json(
            { error: 'Observation not found or access denied' },
            { status: 404 }
        ...
        logAuthEvent(
          'LOGIN_SUCCESS',
          user.id,
          `Deleted observation: ${deletedObservation.title}`
        );
